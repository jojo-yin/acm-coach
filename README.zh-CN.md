...
| 场景 | 能力 |
|------|------|
| 题目没思路 | 四阶段工作流（READ → THINK → CODE → VERIFY）——约束分析、算法选择、引导式实现 |
| WA / TLE / RE 了 | 系统化诊断，精准定位到行。附带对拍脚本，自动发现反例 |
| 代码需要审查 | 复杂度审计、正确性检查、边界情况枚举。尊重 ACM 习惯，不挑剔代码风格 |
| 想追踪进步 | 跨 session 自动画像——高频 bug、强弱项、CF 等级评估 |
| 有一批题解文件 | 批量通读目录，生成结构化报告：高频错误、强项、改进优先级 |
| 使用 Codeforces | 绑定 CF 账号——获取 rating 走势、按 tag 分析 AC 率、查看近期比赛 |
| 需要速查参考 | 28 个 C++ 算法模板（并查集、线段树、Dijkstra、KMP、LCA、SCC、组合数）+ 16 类常见陷阱 |
...
- **帮助菜单。** 问「你能做什么」现在展示 6 个路径菜单（A–F），无需猜测触发词。
- **功能表格。** 改为双列 `场景 | 能力` 专业布局，去掉颜文字。
- **更新日志。** 在 README（中英文）中记录版本发布内容。
