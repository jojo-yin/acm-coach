...
| 你需要什么 | 教练能做什么 |
|-----------|------------|
| 🤔 题目没思路 | 四阶段流程（READ → THINK → CODE → VERIFY）——先分析约束定复杂度预算，再推荐算法，用提问引导思考，不直接给答案 |
| 🐛 WA / TLE / RE 了 | 精准定位到出错行和根因——不是瞎改。含对拍脚本，自动找反例 |
| ✅ 代码写完了，想让人看看 | 审查复杂度、正确性、边界情况。不挑 `#include <bits/stdc++.h>` 或大括号风格 |
| 📊 想知道自己哪里弱 | 跨 session 自动追踪 bug 模式，生成水平等级（CF rank）和知识点强弱图。毕业门机制过滤偶发失误 |
| 📁 有一堆题解代码 | 批量通读你的文件夹，生成报告：高频错误 Top 3、强项、下一步重点练什么 |
| 🔗 有 Codeforces 账号 | 绑定 CF 号，查 rating 走势，按 tag 统计 AC 率，查看即将开始的比赛 |
| 🧠 需要算法模板 | 28 个即用 C++ 模板：并查集、线段树、Dijkstra、KMP、LCA、SCC、快速幂、组合数等 |
| 🪤 反复踩同样的 C++ 坑 | 16 类常见错误，附修改前后对比：溢出、越界、遍历中 erase、unordered_map 被卡等 |
| 👥 ICPC 三人组队 | 三人一机策略：Think Tank 模型、角色分工、比赛阶段、纸上调试法 |
